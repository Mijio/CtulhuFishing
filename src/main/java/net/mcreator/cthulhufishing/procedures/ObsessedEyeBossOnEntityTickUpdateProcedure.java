			...
			}.compareDistOf(x, y, z)).findFirst().orElse(null)) == null)) {
				{
					...
					for (Entity entityiterator : _entfound) {
						if (!(entityiterator instanceof ObsessedEyeBossEntity) && !(entityiterator instanceof ObsessedEyeMinionEntity)) {
							...
							if (world instanceof ServerLevel _level)
								_level.sendParticles(ParticleTypes.PORTAL, (entityiterator.getX()), (entityiterator.getY()), (entityiterator.getZ()), 60, 0.5, 0.5, 0.5, 0);
						}
					}
				}
			}
		...
		if (entity.getPersistentData().getDouble("AI") == 200) {
			if (!((entity instanceof Mob _mobEnt ? (Entity) _mobEnt.getTarget() : null) == null)) {
				entity.setDeltaMovement(new Vec3((((entity instanceof Mob _mobEnt ? (Entity) _mobEnt.getTarget() : null).getX() - entity.getX()) * 0.2),
						(((entity instanceof Mob _mobEnt ? (Entity) _mobEnt.getTarget() : null).getY() - entity.getY()) * 0.2), (((entity instanceof Mob _mobEnt ? (Entity) _mobEnt.getTarget() : null).getZ() - entity.getZ()) * 0.2)));
				dx = x - 1;
				dy = y - 1;
				dz = z - 1;
